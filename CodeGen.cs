...
public class CodeGenerator {
	...
	public void Emit (Op op, int val) {
		Emit(op); Put(val>>8); Put(val); operSum = operSum + 1;
		}

	public void Emit(Op op, double val) {
		Emit(op); operSum++;
		}
	public void Emit(Op op, bool val) {
		Emit(op); operSum++;
		}
		public void Emit(Op op, string val) {
		Emit(op); operSum++;
		}
